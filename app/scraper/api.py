...
def get_rcr(request):
    ...
    fields_type = {
        k.name: k
        for k in Rcr._meta.get_fields()
        if k.get_internal_type() != "ForeignKey"
    }
            ...
            try:
                for index, field in enumerate(fields):
                    ...
                    try:
                        ...
                        if hasattr(Rcr, f"{field}_from_list") and callable(
                            getattr(Rcr, f"{field}_from_list")
                        ):
            ...
            try:
                ...
                for i in fields:
                    ...
                    if i in ("latitude", "longitude"):
                        attr_in_base = (
                            round(Decimal(getattr(obj_in_base, i)), 8)
                            if getattr(obj_in_base, i) is not None
                            else None
                        )
                        attr_in_list = (
                            round(Decimal(obj[i]), 8) if obj[i] is not None else None
                        )
                    else:
...
def get_lang(request):
    ...
    return (
        200,
        {"created": created, "updated": updated, "errors": 0},
    )
...
@router.post(
    ...
    response={
        200: List[numberResponseSchema],
        codes_4xx: errorSchema,
        500: errorSchema,
    },
)
def get_sudoc(request, query: SudocQuerySpecificSchema):
    ...
    search_keys = {}
    query_class_fields = [
        i for i in SudocQuery._meta.get_fields() if i.name not in IGNORE_ATTRIBUTES
    ]
    try:
    ...
    for url_data in product(*foreign_data):
        ...
        database_criterias = {
            i.name: None if i.default == NOT_PROVIDED else i.default
            for i in query_class_fields
        }
        query_parameters = {}
        for i, j in zip(foreign_lib, url_data):
            database_criterias[i] = j
            query_parameters[i] = (
                getattr(j, QUERY_SUBFIELDS[i]) if i in QUERY_SUBFIELDS else j
            )
        query_string = "%20and%20".join(
            [f"{k}%3D{v}" for k, v in query_parameters.items()]
        )
        logger.debug(f"{database_criterias=}")
        sudoc_query = SudocQuery.objects.filter(
            **{**database_criterias, **{"created": today}}
        )
        if sudoc_query.exists():
            logger.warning(
                f"Query {query_string.replace('%20and%20', ',').replace('%3D', '=')} for {today} already exists"
            )
        else:
            url = url_base.format(query=query_string)
            logger.debug(url)
            t = Thread(
                target=SudocQuery.get_from_sudoc,
                kwargs={
                    "url": url,
                    "criterias": database_criterias,
    ...
    if start_thread:
        ...
        return 200, [
            {"language": i.lan.code, "rcr": i.rbc.rcr, "number": i.number}
